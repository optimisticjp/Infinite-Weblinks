...
| ------------------------------------------------- | -------- | ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `next`                                            | high     | **yes**                             | Most advisories are **N/A**: build is `next build --webpack` (not Turbopack), there are **no Server Actions** (`grep "use server"` → none) and **no middleware**, and images are `unoptimized` (Image Optimization API unused). Rewrites are static (no attacker-controlled destination → SSRF-in-rewrites N/A). | **Phase 3C: applied** — scoped bump `16.2.10 → 16.2.11` (latest patch, no `--force`, full re-validation). It clears Next.js's **own 9 high-severity advisories** (middleware bypass, cache confusion, SSRF-in-rewrites, image-opt DoS, etc.); it does **not** clear the bundled `postcss`/`sharp` (see those rows). **[verified-locally]** |
| `postcss`                                         | moderate | **yes**                             | XSS via CSS stringify output; a **build-time** tool over our own authored CSS (not attacker-controlled). Transitive via `next`.                                                                                                                                                                                  | **Phase 3C correction:** NOT cleared by the `next@16.2.11` bump — `postcss` is bundled inside `next` at every available version (npm's only "fix" is a breaking downgrade to `next@9.3.3`, rejected). Now rated **high**. Build-time-only over our own CSS; low real risk; no forward fix. **[reasoned]**                                  |
...
Totals recorded (Phase 3A, `next@16.2.10`): full audit **6 (1 moderate, 5 high)**; production
(`--omit=dev`) **3 (1 moderate, 2 high)** — `next`, `postcss`, `sharp`.

**Phase 3C update (`next@16.2.11`, re-run):** full audit **7 (2 moderate, 5 high)**; production
(`--omit=dev`) **3 high** (`postcss`, `sharp`, and `next` only as their parent). The bump **cleared
Next.js's own 9 high-severity advisories**; the residual roots are exactly `postcss` (bundled in
`next`; build-time, over our own CSS) and `sharp` (transitive; **no Worker-runtime exposure** —
`images.unoptimized`, no sharp on Workers). Neither has a forward fix (npm's only offer is a breaking
`next@9.3.3` downgrade, rejected). The 2 "moderate" are `@opennextjs/aws` / `@opennextjs/cloudflare`
**inheritance nodes** over the same `postcss`/`sharp` roots, not new advisories; `wrangler`/`miniflare`
remain a **dev-only**, scheduled upgrade. See `docs/release/phase-3c-owner-decisions.md` and the
Phase 3C release certification for the current state.
